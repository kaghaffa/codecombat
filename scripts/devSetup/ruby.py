...
class Ruby(dependency.Dependency):
    ...
    def __init__(self, config):
        ...
        if is_ruby_installed and not is_gem_installed:
            # this means their ruby is so old that RubyGems isn't packaged with
            # it
            raise errors.CoCoError(
                u"You have an extremely old version of Ruby. Please upgrade it to get RubyGems.")
    ...
    def install_ruby(self):
        ...
        if operating_system == u"windows":
            ...
            raise errors.CoCoError(
                u"Please install Ruby on your Linux distribution(try 'sudo apt-get install ruby'.")
