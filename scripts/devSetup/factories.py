...
class SetupFactory(object):
    ...
    def setup(self):
        ...
        try:
            mongo_version_string = subprocess.check_output(
                "mongod --version",
                shell=True)
        except:
        ...
        bower_path = self.config.directory.root_dir + os.sep + "coco" + \
            os.sep + "node_modules" + os.sep + ".bin" + os.sep + "bower"
        subprocess.call(
            bower_path +
            " --allow-root install",
            shell=True,
        ...
        # TODO: Make this more robust and portable(doesn't pose security risk
        # though)
        subprocess.call(
            "chmod -R 755 " +
            self.config.directory.root_dir +
            os.sep +
            "coco" +
            os.sep +
            "bin",
        ...
        print(
            "Done! If you want to start the server, head into /coco/bin and run ")
        print("1. ./coco-mongodb")
        print("2. ./coco-brunch ")
        print("3. ./coco-dev-server")
        print(
            "NOTE: brunch may need to be run as sudo if it doesn't work (ulimit needs to be set higher than default)")
