    ...
    def __progress_bar_reporthook(self, blocknum, blocksize, totalsize):
        ...
        if totalsize > 0:
            progress_fraction = float(
                amount_of_data_downloaded_so_far) / float(totalsize)
            progress_percentage = progress_fraction * 1e2
            stringToDisplay = '\r[{0}] {1:.1f}%'.format(
                '#' * int(bars_to_display * progress_fraction),
                progress_percentage)
            print stringToDisplay,
            if amount_of_data_downloaded_so_far >= totalsize:
                print "\n",
        else:
            stringToDisplay = '\r File size unknown. Read {0} bytes.'.format(
                amount_of_data_downloaded_so_far)
            print stringToDisplay,
