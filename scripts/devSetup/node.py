...
class Node(Dependency):
    ...
    def __init__(self, configuration):
        ...
        if operating_system == u"mac":
            ...
            # TODO: Fix windows node installation
            self.config.directory.create_directory_in_bin(u"node")
    ...
    def install_dependencies(self):
        ...
        if self.config.system.operating_system in ["mac", "linux"] and not which("node"):
            ...
            shutil.copy(
                ...
                "node",
                "/usr/local/bin/")
            os.chmod("/usr/local/bin/node", 0o777)
        ...
        if self.check_if_executable_installed(u"npm"):
            ...
            # for bash script, you have to somehow redirect stdin to
            # raw_input()
        ...
        if wants_to_upgrade:
            ...
            npm_install_script_path  = install_directory + \
                os.sep + u"install.sh"
            urllib.urlretrieve(
                u"https://npmjs.org/install.sh",
                filename=npm_install_script_path)
...
class LinuxNodeDownloader(NodeDownloader):
    ...
    def download_url(self):
        if self.dependency.config.mem_width == 64:
            return (
                u"http://nodejs.org/dist/v0.10.24/node-v0.10.24-linux-x64.tar.gz"
            )
        else:
            return (
                u"http://nodejs.org/dist/v0.10.24/node-v0.10.24-linux-x86.tar.gz"
            )
...
class MacNodeDownloader(NodeDownloader):
    ...
    def download_url(self):
        if self.dependency.config.mem_width == 64:
            return (
                u"http://nodejs.org/dist/v0.10.24/node-v0.10.24-darwin-x64.tar.gz"
            )
        else:
            return (
                u"http://nodejs.org/dist/v0.10.24/node-v0.10.24-darwin-x86.tar.gz"
            )
