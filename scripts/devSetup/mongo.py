...
class MongoDB(Dependency):
    ...
    def bashrc_string(self):
        return (
            "COCO_MONGOD_PATH=" + self.config.directory.bin_directory +
            os.sep + u"mongo" + os.sep + "bin" + os.sep + "mongod"
        )
    ...
    def install_dependencies(self):
        install_directory = self.config.directory.bin_directory + \
            os.sep + u"mongo"
        import shutil
    ...
    def findUnzippedMongoBinPath(self):
        return self.downloader.download_directory + os.sep + \
            (os.walk(self.downloader.download_directory)
                .next()[1])[0] + os.sep + u"bin"
...
class LinuxMongoDBDownloader(MongoDBDownloader):
    ...
    def download_url(self):
        if self.dependency.config.mem_width == 64:
            return (
                u"http://fastdl.mongodb.org/linux/mongodb-linux-x86_64-2.5.4.tgz"
            )
        else:
            warnings.warn(
                u"MongoDB *really* doesn't run well on 32 bit systems. You have been warned.")
            return (
                u"http://fastdl.mongodb.org/linux/mongodb-linux-i686-2.5.4.tgz"
            )
...
class WindowsMongoDBDownloader(MongoDBDownloader):
    ...
    def download_url(self):
        ...
        warnings.warn(
            u"If you have a version of Windows older than 7, MongoDB may not function properly!")
        if self.dependency.config.mem_width == 64:
            return (
                u"http://fastdl.mongodb.org/win32/mongodb-win32-x86_64-2008plus-2.5.4.zip"
            )
        else:
            return (
                u"http://fastdl.mongodb.org/win32/mongodb-win32-i386-2.5.4.zip"
            )
